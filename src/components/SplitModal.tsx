...
export const SplitModal: React.FC<SplitModalProps> = ({
  ...
  const handleSplitEveryN = () => {
    const chunk = Math.max(1, everyN)
    const newParts: SplitPartConfig[] = []
    let partIdx = 1

    for (let i = 0; i < totalPages; i += chunk) {
      const end = Math.min(totalPages, i + chunk)
      const indices: number[] = []
      for (let p = i; p < end; p++) {
        indices.push(p)
      }
      const label = i + 1 === end ? `Page ${i + 1}` : `Pages ${i + 1}-${end} (${indices.length} pages)`
      newParts.push({
        id: `part_n_${partIdx}_${Date.now()}`,
        name: `Part_${partIdx}`,
        pageIndices: indices,
        rangeLabel: label,
      })
  ...
  // Quick preset: Split Even & Odd Pages into 2 files
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="bg-[#0b111e] border border-slate-700/80 rounded-2xl max-w-4xl w-full max-h-[92vh] sm:max-h-[90vh] flex flex-col shadow-2xl text-slate-100 overflow-hidden">
        {/* Header (Fixed) */}
        <div className="flex items-center justify-between border-b border-slate-800/80 p-4 sm:p-6 shrink-0">
          <div className="flex items-center space-x-3 min-w-0">
            <div className="w-9 h-9 rounded-xl bg-sky-500/20 text-sky-400 flex items-center justify-center shrink-0">
              <Scissors className="w-5 h-5" />
            </div>
            <div className="min-w-0">
              <h2 className="text-base sm:text-lg font-bold text-slate-100 tracking-tight flex items-center space-x-2 truncate">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 shrink-0">
            <span className="px-2.5 py-1 bg-sky-950/60 border border-sky-800/60 text-sky-400 text-xs font-semibold rounded-lg shadow-inner">
              {totalPages} Pages
            </span>
            <button
              onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
            ...
            <div className="md:col-span-7 flex flex-col bg-[#0d131f] border border-slate-800 rounded-xl p-4 min-h-[300px] h-[380px]">
        ...
        <div className="p-4 sm:p-5 border-t border-slate-800/80 bg-[#0b1324] flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 shrink-0">
          <button
            type="button"
            onClick={handleClearAllRanges}
            disabled={parts.length === 0 || isExporting}
            className="inline-flex items-center justify-center space-x-1.5 px-3.5 py-2 bg-[#ef4444] hover:bg-[#dc2626] disabled:opacity-40 text-white text-xs font-semibold rounded-xl shadow transition-colors"
          >
            <Trash2 className="w-4 h-4" />
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <div className="flex items-center space-x-2 justify-end">
              <button
                ...
                className="flex-1 sm:flex-initial px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold rounded-xl transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                disabled={parts.length === 0 || isExporting}
                className="flex-1 sm:flex-initial inline-flex items-center justify-center space-x-2 px-5 py-2 bg-[#0284c7] hover:bg-[#0369a1] disabled:bg-slate-800 disabled:text-slate-500 disabled:opacity-50 text-white text-xs font-bold rounded-xl shadow-md shadow-sky-950/40 transition-colors"
              >
